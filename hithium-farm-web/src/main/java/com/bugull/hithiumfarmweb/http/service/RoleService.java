...
public class RoleService {
    ...
    public ResHelper<List<RoleEntity>> select(SysUser sysUser) {
        if (sysUser.getId()== 1L) {
            List<RoleEntity> results = roleEntityDao.findBatch(new Query());
            return ResHelper.success("", results);
        }
        if(!CollectionUtils.isEmpty(sysUser.getRoleIds()) && !sysUser.getRoleIds().isEmpty()){
            List<RoleEntity> roleEntities = roleEntityDao.findBatch(new Query().addCriteria(Criteria.where("_id").in(sysUser.getRoleIds())));
            return ResHelper.success("",roleEntities);
        }
        return null;
    }

//    public ResHelper<Page<RoleEntity>> query(Map<String, Object> params) {
//
